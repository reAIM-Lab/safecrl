...
class SyntheticDataContinuousInt:
    def __init__(self, c_dim=5, u_dim=5, x_dim=1, y_dim=1, n=1000):
        ...
        self.alpha_c = np.asarray([0.6585762, 0.85822724]).reshape((self.c_dim, 1))
        # self.beta_c = np.random.uniform(+0.1, 0.5, size=(self.c_dim, 1))
        self.beta_c = np.asarray([0.34110535, 0.31795327]).reshape((self.c_dim, 1))
        ...
        self.alpha_x = np.asarray([-0.03744767, 0.012350638, 0.0127819766])
        ...
        self.alpha_y = np.asarray([-0.71579547, -0.57289624, -0.66488678, 3.5])
    ...
    def load_simulated_toy_highdim_cont(self):
        ...
        self.data_mat[:, self.u_dim + self.c_dim] = np.double(
            expit(np.dot(self.data_mat[:, :self.u_dim + self.c_dim], self.alpha_x) + np.random.normal(0,scale=1,size=self.data_mat.shape[0]) - self.x_th) > 0.5)
        ...
        y_x0 = np.double(
            expit(np.dot(
                np.concatenate((self.data_mat[:, :self.u_dim + self.c_dim], np.zeros((self.data_mat.shape[0], 1))),
                               axis=1), self.alpha_y) - self.y_th) > 0.5)
        y_x1 = np.double(
            expit(np.dot(
        ...
        for i in [0, 1]:
            for j in [0, 1]:
                print('i:', i, 'j:', j, len(np.intersect1d(np.where(y_x0 == i)[0], np.where(y_x1 == j)[0])))
